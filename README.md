...
| `android-cli` | [Google's Android CLI](https://developer.android.com/tools/agents/android-cli) — the agent-first `android` command (scaffold projects, manage AVDs, run Journeys, `android init` to register its agent skill). Downloads the prebuilt binary to `~/.local/bin` and puts it on PATH; a lightweight, standalone one-token install (`android` implies it, but it does **not** pull the Nix SDK back) | `dl.google.com` | — |
