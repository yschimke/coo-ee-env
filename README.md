...
| `android-cli` | [Google's Android CLI](https://developer.android.com/tools/agents/android-cli) — the agent-first `android` command (scaffold projects, manage AVDs, run Journeys). Downloads the prebuilt binary to `~/.local/bin`, puts it on PATH, and runs `android init` to register its agent skill (opt out with `COOEE_ANDROID_CLI_INIT=0`); a lightweight, standalone one-token install (`android` implies it, but it does **not** pull the Nix SDK back) | `dl.google.com` | — |
